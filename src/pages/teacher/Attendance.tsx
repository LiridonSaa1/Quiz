...
import {
  ...
  XCircle, Clock, AlertTriangle, X, Pencil, Trash2, Calendar, Mail,
} from 'lucide-react';
...
const emptyForm = {
  ...
  notifyParents: false, emailLang: 'sq' as 'sq' | 'en',
};
...
export default function TeacherAttendance() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        // Edits go direct — no email on update
        ...
        // New record — call server endpoint so it can send parent email
        const res = await authFetch('/api/teacher/attendance', {
          ...
          body: JSON.stringify({
            ...
            emailLang: form.emailLang,
          }),
        });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(json?.error || t('teacher.attendance.failedSave'));
        toast.success(t('teacher.attendance.attendanceMarked'));
        if (form.notifyParents) toast.success(t('teacher.attendance.emailWillBeSent'));
      }
      setShowModal(false);
      fetchData();
    } catch (e: any) {
        ...
        <div className="fixed inset-0 lg:left-60 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              {/* Parent email notification — only on new records */}
              {!editId && (
                  ...
                  <label className="flex items-center gap-3 cursor-pointer select-none">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-slate-700">
                        {t('teacher.attendance.notifyParents')}
                      </span>
                    </div>
                  </label>
                  ...
                  {form.notifyParents && (
                    <div className="pl-7 space-y-2">
                      <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block">
                        {t('teacher.attendance.emailLanguage')}
                      </label>
                      <div className="flex gap-2">
                        {(['sq', 'en'] as const).map(lang => (
                          <button
                            key={lang}
                            type="button"
                            onClick={() => setForm(f => ({ ...f, emailLang: lang }))}
                            className={cn(
                          ...
                          >
                            {lang === 'sq'
                              ? t('teacher.attendance.emailLangAlbanian')
                              : t('teacher.attendance.emailLangEnglish')}
                          </button>
                        ))}
                      </div>
                      <p className="text-xs text-indigo-600 font-medium flex items-center gap-1">
                        <Mail className="w-3 h-3" />
                        {t('teacher.attendance.emailWillBeSent')}
                      </p>
                    </div>
                  )}
                </div>
