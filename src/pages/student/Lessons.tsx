...
const TYPE_CFG = (t: any): Record<string, { label: string; icon: React.ElementType; color: string; bg: string; border: string }> => ({
  video: { label: t('common.video'), icon: Video, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
  text:  { label: t('common.text'),  icon: FileText, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100' },
  quiz:  { label: t('common.quiz'),  icon: HelpCircle, color: 'text-violet-600', bg: 'bg-violet-50', border: 'border-violet-100' },
});
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                  <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-3 py-1.5 mb-3">
                    ...
                    <span className="text-white/85 text-xs font-semibold">{t('nav.studentPortal')}</span>
                  </div>
                  <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">{t('student.lessons.myLessons')}</h1>
                  <p className="text-indigo-200 text-sm mt-2 max-w-md">
                    {t('student.lessons.lessonsSummary', { count: lessons.length, hours, mins })}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative flex-1 min-w-[180px]">
                ...
                <input
                  ...
                  placeholder={t('student.lessons.searchPlaceholder')}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    typeFilter === t_key
                      ...
                      : 'bg-white/60 text-slate-500 border-slate-100 hover:border-indigo-200'
                  )}
                >
            ...
            {loading ? (
              ...
              >
                ...
                <h3 className="text-xl font-extrabold text-slate-800 mt-4 mb-2">
                  {hasActiveFilters ? t('student.lessons.noResultsFound') : t('student.lessons.noLessonsYet')}
                </h3>
                <p className="text-slate-400 text-sm max-w-xs text-center">
                  {hasActiveFilters ? t('student.lessons.tryAdjustingFilters') : t('student.lessons.enrollToAccess')}
                </p>
              </motion.div>
            ) : (
              <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <AnimatePresence>
                  {filtered.map((lesson) => {
                    const typeCfg = TYPE_CFG(t);
                    const cfg = typeCfg[lesson.type] || typeCfg.text;
                    const Icon = cfg.icon;
                        ...
                        <div className="p-5 flex flex-col flex-1">
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <span className={cn(
                              ...
                              {isCompleted ? t('common.completed') : locked ? t('common.locked') : t('common.open')}
                            </span>
                          </div>
                          ...
                          <div className="mt-auto space-y-2 pt-3 border-t border-slate-50">
                            <div className="flex items-center justify-between">
                              <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-100 text-slate-600 rounded-lg text-[11px] font-medium max-w-[130px] truncate">
                                <span className="truncate">{lesson.moduleTitle || t('student.lessons.generalModule')}</span>
                              </span>
                              <span className="inline-flex items-center gap-1 text-xs text-slate-400">
                                <Clock className="w-3.5 h-3.5 text-slate-300" />
                                {t('common.minutesCount', { count: lesson.durationMinutes })}
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-[11px] font-semibold text-slate-400 truncate">{lesson.courseTitle}</span>
                              <span className={cn('text-[10px] font-bold px-2 py-1 rounded-full', cfg.bg, cfg.color)}>{t(`common.${lesson.type}`)}</span>
                            </div>
                            <Link
                            ...
                            >
                              ...
                              {isCompleted ? t('common.review') : locked ? t('common.viewDetails') : isStarted ? t('common.continue') : t('common.start')}
