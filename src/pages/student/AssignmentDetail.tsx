...
import {
  ...
  GraduationCap, Tag, Info, Star, CalendarCheck, Lock,
} from 'lucide-react';
...
const TYPE_META: Record<string, { label: string; color: string; bg: string; border: string; icon: string }> = {
  homework: { label: 'Homework',  color: 'text-blue-700',   bg: 'bg-blue-50',   border: 'border-blue-200',   icon: '📝' },
  project:  { label: 'Project',   color: 'text-violet-700', bg: 'bg-violet-50', border: 'border-violet-200', icon: '🚀' },
  essay:    { label: 'Essay',     color: 'text-rose-700',   bg: 'bg-rose-50',   border: 'border-rose-200',   icon: '✍️' },
  quiz:     { label: 'Quiz',      color: 'text-amber-700',  bg: 'bg-amber-50',  border: 'border-amber-200',  icon: '❓' },
  lab:      { label: 'Lab',       color: 'text-teal-700',   bg: 'bg-teal-50',   border: 'border-teal-200',   icon: '🔬' },
  other:    { label: 'Other',     color: 'text-slate-600',  bg: 'bg-slate-100', border: 'border-slate-200',  icon: '📋' },
};
...
export default function StudentAssignmentDetail() {
    ...
    const load = async () => {
      ...
      if (courses.length > 0) {
        ...
        if (rows?.[0]) {
          ...
          assignmentRow._courseTitle = courseMap[assignmentRow.course_id] || 'Course';
        }
      }
      ...
      if (!assignmentRow) {
        try {
          ...
          const found = (json.assignments || []).find((a: any) => String(a.id) === String(assignmentId));
          if (found) {
            assignmentRow = found;
            assignmentRow._courseTitle = found.course_title || found.courseTitle || '';
          }
        } catch { }
      }

      if (!assignmentRow) { setLoading(false); return; }

      let parsedConfig: SubmissionConfig | null = null;
      if (assignmentRow.submission_config) {
        try {
          parsedConfig = typeof assignmentRow.submission_config === 'string'
      ...
      setAssignment({
        ...
        max_score: assignmentRow.max_score == null ? null : Number(assignmentRow.max_score),
        status: assignmentRow.status || 'published',
        course_id: assignmentRow.course_id ? String(assignmentRow.course_id) : null,
        course_title: assignmentRow._courseTitle || assignmentRow.course_title || '',
  ...
  const handleUploadFiles = async (fileList: FileList | File[]) => {
    ...
    for (const file of arr) {
      ...
      try {
        ...
        const { data, error } = await supabase.storage.from('assignment-files').upload(path, file, { upsert: false });
  ...
  const handleSubmit = async () => {
    ...
    const validLinks = links.filter(l => l.url.trim()).map(l => ({ url: l.url.trim(), label: l.label.trim() || l.url.trim() }));
  ...
  const typeMeta = TYPE_META[(assignment?.type || 'other').toLowerCase()] || TYPE_META.other;
  ...
  const openEditForm = () => {
    if (submission) {
      ...
      setLinks(submission.link_urls?.length ? submission.link_urls : [{ url: '', label: '' }]);
    }
    setShowForm(true);
  };
  ...
  return (
    <StudentLayout>
      <div className="max-w-3xl mx-auto space-y-5 pb-8">
        ...
        {/* Back link */}
        <Link to="/student/assignments" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-teal-600 transition-colors group">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-16 text-center shadow-sm">
            ...
            <p className="text-sm text-slate-400 mt-1">{t('student.assignments.noAccessAssignment')}</p>
            <Link to="/student/assignments" className="mt-5 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-slate-200 transition-colors">
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    {assignment.course_title && (
                      <>
                        <span className="inline-flex items-center gap-1.5 text-white/60 text-xs font-semibold">
                          <BookOpen className="w-3.5 h-3.5" /> {assignment.course_title}
                        </span>
                    ...
                    <span className={cn(
                      ...
                      typeMeta.bg.replace('bg-', 'bg-opacity-20 bg-'), 'border-white/20 text-white'
                    )}>
                  ...
                  {assignment.description && (
                    <p className="text-white/75 text-sm leading-relaxed mb-4 max-w-xl">{assignment.description}</p>
                  )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-slate-100">
                <div className="px-5 py-4 text-center">
                  <div className={cn('text-[10px] font-bold uppercase tracking-wider mb-1.5', isExpired ? 'text-rose-400' : 'text-slate-400')}>
                    {isExpired ? 'Overdue' : 'Due Date'}
                  </div>
                  <div className={cn('text-sm font-black', isExpired ? 'text-rose-600' : 'text-slate-800')}>
                ...
                <div className="px-5 py-4 text-center">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Max Score</div>
                ...
                <div className="px-5 py-4 text-center">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">My Status</div>
                  <div className={cn('text-sm font-black flex items-center justify-center gap-1',
                    submission?.status === 'graded' ? 'text-emerald-600' :
                    submission ? 'text-blue-600' : 'text-slate-400')}>
                    {submission?.status === 'graded' ? (
                ...
                <div className="px-5 py-4 text-center">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Assignment</div>
                  <div className={cn('text-sm font-black px-2 py-0.5 rounded-lg inline-flex items-center gap-1', typeMeta.bg, typeMeta.color)}>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
              ...
              <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-700">Assignment Details</h2>
              </div>
              <div className="divide-y divide-slate-50">
                {[
                  ...
                  },
                  {
                    ...
                    label: 'Type',
                    value: (
                      <span className={cn('inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-lg border', typeMeta.bg, typeMeta.color, typeMeta.border)}>
                        <span>{typeMeta.icon}</span> {typeMeta.label}
                      </span>
                    ),
                  },
                  ...
                  },
                  {
                    ...
                    label: 'Max Score',
                    value: assignment.max_score != null ? `${assignment.max_score} points` : 'Not specified',
                  },
                  {
                    ...
                    value: assignment.allow_late_submission
                      ...
                      : <span className="text-rose-500 font-semibold">Not allowed</span>,
                  },
                  {
                    ...
                    value: (
                      <span className={cn('inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-lg',
                        ...
                        assignment.status === 'closed' ? 'bg-amber-50 text-amber-700' :
                        'bg-slate-100 text-slate-600'
                      )}>
                        {assignment.status === 'published' ? <CheckCircle2 className="w-3 h-3" /> : <Archive className="w-3 h-3" />}
                        {assignment.status.charAt(0).toUpperCase() + assignment.status.slice(1)}
                      </span>
                    ),
                  },
                  ...(assignment.created_at ? [{
                    icon: <CalendarCheck className="w-4 h-4 text-slate-400" />,
                    label: 'Published',
                    value: format(new Date(assignment.created_at), 'MMM d, yyyy'),
                  }] : []),
                ].map((row, i) => (
                  <div key={i} className="flex items-center gap-4 px-6 py-3.5">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center shrink-0">{row.icon}</div>
                    <div className="text-xs font-semibold text-slate-400 w-28 shrink-0">{row.label}</div>
            ...
            {assignment.instructions && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-slate-700">{t('common.instructions')}</h2>
                </div>
            ...
            {submission?.status === 'graded' && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="px-6 py-4 border-b border-emerald-50 flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-slate-700">{t('student.assignments.yourGrade')}</h2>
                </div>
                <div className="px-6 py-5">
                    ...
                    {submission.grade != null && (
                      <div className="text-center shrink-0">
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 flex flex-col items-center justify-center shadow-lg shadow-emerald-200/60">
                          <div className="text-3xl font-black text-white leading-none">{submission.grade}</div>
                          <div className="text-white/70 text-[10px] font-semibold mt-0.5">pts</div>
                        </div>
                        <div className="text-xs text-slate-400 mt-2 font-medium">out of {assignment.max_score ?? '?'}</div>
                      </div>
                    )}
                    {submission.feedback && (
                ...
                <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-bold text-slate-700">
                      {submission ? t('student.assignments.yourSubmission') : t('student.assignments.submitWork')}
                    </h2>
                  </div>
                  {submission && !showForm && (
                      ...
                      {submission.content && (
                        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('student.assignments.textAnswer')}</span>
                          </div>
                          ...
                          <div className="space-y-2">
                            {submission.file_urls.map((f, i) => (
                              <a key={i} href={f.url} target="_blank" rel="noopener noreferrer"
                                ...
                                <span className="text-slate-400 group-hover:text-blue-500">{getFileIcon(f.mime_type)}</span>
                          ...
                          <motion.div key="files" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
                            ...
                            {files.map((f, i) => (
                              <div key={i} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100">
                                ...
                                <a href={f.url} target="_blank" rel="noopener noreferrer" className="p-1 text-slate-400 hover:text-blue-500"><ExternalLink className="w-3.5 h-3.5" /></a>
                                <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="p-1 text-slate-400 hover:text-rose-500"><X className="w-3.5 h-3.5" /></button>
                              </div>
                            ))}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link to="/student/assignments"
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm transition-colors">
                <ClipboardList className="w-4 h-4 text-slate-400" /> All assignments
              </Link>
            </div>
          </>
        )}
