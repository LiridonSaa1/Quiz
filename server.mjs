...
Assistant:`
  ...
  app.get("/api/teacher/config/settings", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("platform_config").select("value").eq("section", section).maybeSingle();
  ...
  app.put("/api/teacher/config/settings", async (req, res) => {
    try {
      ...
      if (!caller) return;
      const value = req.body?.value;
      if (value === void 0) return res.status(400).json({ error: "value is required" });
      const section = `teacher_settings:${caller.userId}`;
      const { error } = await supabaseAdmin.from("platform_config").upsert({ section, value, updated_at: (/* @__PURE__ */ new Date()).toISOString() }, { onConflict: "section" });
      if (error && !isPlatformConfigMissing(error)) throw error;
      res.json({ success: true });
    } catch (e) {
...
${e?.stack || ""}`),
  ...
  app.post("/api/admin/payments", async (req, res) => {
    try {
      ...
      if (paymentId) {
        ...
        await supabaseAdmin.from("student_monthly_payments").upsert(
          { student_id, month_year: monthYear, amount: numericAmount, notes: String(description || "").trim(), paid_at: (/* @__PURE__ */ new Date()).toISOString() },
      ...
      await supabaseAdmin.from("student_monthly_payments").upsert(
        { student_id, month_year: monthYearNoInv, amount: numericAmount, notes: String(description || "").trim(), paid_at: (/* @__PURE__ */ new Date()).toISOString() },
        { onConflict: "student_id,month_year" }
      ).then(() => {
      });
      await dispatchNotifyEvent("paymentReceived", {
  ...
  app.get("/api/admin/teacher-hours/invoice", async (req, res) => {
    try {
      ...
      const rows = hoursRes.data || [];
      const total_hours = rows.reduce((s2, r) => s2 + Number(r.hours), 0);
      const total_amount = rows.reduce((s2, r) => s2 + Number(r.hours) * Number(r.rate_per_hour), 0);
      const s = settingsRaw || {};
      const business = {
...
${shortContent}`;
  ...
  app.post("/api/teacher/assignments/upload-attachment", async (req, res) => {
    try {
      const caller = await assertAuthenticated(req, res);
      if (!caller) return;
      if (caller.role !== "teacher" && caller.role !== "admin") return res.status(403).json({ error: "Forbidden" });
      ...
      const { data, error } = await supabaseAdmin.storage.from("assignment-attachments").createSignedUploadUrl(storagePath);
      if (error) return res.status(500).json({ error: error.message });
  ...
  app.patch("/api/teacher/assignments/:id", async (req, res) => {
    try {
      ...
      try {
        ...
        if (b.attachments !== void 0) col("attachments", b.attachments != null ? JSON.stringify(b.attachments) : "[]");
  ...
  app.get("/api/teacher/assignments/:assignmentId/submissions", async (req, res) => {
    try {
      ...
      if (usedFallback) {
        const { data: sbRows, error: sbErr } = await supabaseAdmin.from("assignment_submissions").select("*").eq("assignment_id", assignmentId).order("submitted_at", { ascending: false });
        if (sbErr) {
  ...
  app.patch("/api/teacher/assignments/submissions/:subId/grade", async (req, res) => {
    try {
      ...
      try {
        ...
        const gradeVal = grade !== void 0 && grade !== "" ? Number(grade) : null;
        const { data, error: sbErr } = await supabaseAdmin.from("assignment_submissions").update({ grade: gradeVal, feedback: feedback || null, status: "graded", graded_at: now, updated_at: now }).eq("id", subId).select().single();
