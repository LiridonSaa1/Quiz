...
${e?.stack || ""}`),
  ...
  app.get("/api/admin/teacher-hours", async (req, res) => {
    try {
      ...
      const sql = teacher_id ? `SELECT * FROM teacher_hours WHERE work_date >= $1 AND work_date <= $2 AND teacher_id = $3 ORDER BY work_date DESC` : `SELECT * FROM teacher_hours WHERE work_date >= $1 AND work_date <= $2 ORDER BY work_date DESC`;
  ...
  app.patch("/api/admin/teacher-hours/:id", async (req, res) => {
    ...
    try {
      ...
      if (hours !== void 0) {
        ...
        setClauses.push(`hours = $${params.length}`);
      }
      if (rate_per_hour !== void 0) {
        params.push(Number(rate_per_hour));
        setClauses.push(`rate_per_hour = $${params.length}`);
      }
      if (notes !== void 0) {
        params.push(notes);
      ...
      if (work_date !== void 0) {
        ...
        setClauses.push(`work_date = $${params.length}`);
      }
...
${shortContent}`;
  ...
  app.get("/api/student/assignments/:assignmentId/submission", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("assignment_submissions").select("*").eq("assignment_id", assignmentId).eq("student_id", caller.userId).maybeSingle();
  ...
  app.post("/api/student/assignments/:assignmentId/submit", async (req, res) => {
    try {
      ...
      const { data: existing } = await supabaseAdmin.from("assignment_submissions").select("id").eq("assignment_id", assignmentId).eq("student_id", caller.userId).maybeSingle();
      const payload = {
      ...
      if (existing?.id) {
        ...
        result = await supabaseAdmin.from("assignment_submissions").insert({ ...payload, created_at: (/* @__PURE__ */ new Date()).toISOString() }).select().single();
      }
  ...
  app.get("/api/teacher/assignments/:assignmentId/submissions", async (req, res) => {
    try {
      ...
      if (!caller) return;
      const { assignmentId } = req.params;
      const { data: submissions, error } = await supabaseAdmin.from("assignment_submissions").select(`
      ...
      if (error) {
        if (/does not exist|schema cache/i.test(error.message)) {
          const { data: plain } = await supabaseAdmin.from("assignment_submissions").select("id, assignment_id, student_id, submitted_at, score, status, content, file_url, feedback, grade, graded_at").eq("assignment_id", assignmentId).order("submitted_at", { ascending: false });
  ...
  app.patch("/api/teacher/assignments/submissions/:subId/grade", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("assignment_submissions").update({
        grade: grade !== void 0 && grade !== "" ? Number(grade) : null,
