...
When giving instructions, number each step clearly. Be precise and technical when needed.`,
  ...
  app.post("/api/teacher/courses/:courseId/headway-populate", async (req: Request, res: Response) => {
    try {
      ...
      type HLevel = { slug: string; units: HUnit[] };
      const DL = (level_name: string, n: number) => `${OUP}/elt/students/headway/downloads/headway_${level_name}_students_book_unit_${String(n).padStart(2,'0')}.zip`;
      const VL = (level_name: string, n: number) => `${OUP}/elt/students/headway/downloads/headway_${level_name}_video_unit_${String(n).padStart(2,'0')}.zip`;

      const g = (slug: string, unit: string, file: string) => `/student/headway/${slug}/grammar/${unit}/${file}`;
      const v = (slug: string, unit: string, file: string) => `/student/headway/${slug}/vocabulary/${unit}/${file}`;

      const HEADWAY_FULL_DATA: Record<string, HLevel> = {
        "Beginner": {
          slug: "beg",
          units: [
