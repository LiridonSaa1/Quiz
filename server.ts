...
export async function createApp(options: CreateAppOptions = {}) {
  ...
  app.get("/manifest.json", async (_req, res) => {
    try {
      ...
      const pwa: any = b?.pwa && typeof b.pwa === "object" ? b.pwa : {};
      const schoolName =
        (typeof pwa.name === "string" && pwa.name.trim()) ||
      ...
      const shortName =
        ...
        (schoolName.length > 14 ? schoolName.slice(0, 14) : schoolName);
      const description =
        (typeof pwa.description === "string" && pwa.description.trim()) ||
        `${schoolName} — Education Platform`;
      const primaryColor =
        (typeof pwa.themeColor === "string" && pwa.themeColor.trim()) ||
        (typeof b?.colors?.primary === "string" && b.colors.primary) ||
        "#4f46e5";
      const bgColor =
        (typeof pwa.backgroundColor === "string" && pwa.backgroundColor.trim()) ||
        (typeof b?.colors?.sidebar_bg === "string" && b.colors.sidebar_bg) ||
        "#0f172a";
...
When giving instructions, number each step clearly. Be precise and technical when needed.`,
  ...
  /** Admin: reset any user's password directly (no email required). */
  ...
  /** Admin: list all users (all roles) for the configurations panel. */
    ...
    try {
      ...
      if (teacherCourseIds.length > 0) {
        try {
          ...
          const assignmentIds = asgR.rows.map((a: any) => String(a.id));
        ...
        } catch { /* assignments/submissions tables may not exist yet */ }
      }
...
- explanation is one concise sentence`;
  ...
  app.delete('/api/admin/payments/:id', async (req, res) => {
    try {
      ...
      // Fetch payment details before deleting (for cascade)
      ...
      // Cascade: remove student_monthly_payments mirror for this month
      if (pmtRow?.student_id && pmtRow?.payment_date) {
        const monthYear = String(pmtRow.payment_date).slice(0, 7);
        await supabaseAdmin
          .from('student_monthly_payments')
          .delete()
          .eq('student_id', pmtRow.student_id)
...
Content:\n"""${clipped}"""`;
  ...
  app.get('/api/student/lessons/:lessonId/detail', async (req, res) => {
    try {
      ...
      // Multi-path enrollment check (same pattern as /api/student/lessons)
      let enrolledInCourse = caller.role === 'admin';
      let courseTitleForLesson = 'Course';
      if (!enrolledInCourse) {
        // Path A: direct student_ids
        ...
        if (!directCheck.error && directCheck.data) { enrolledInCourse = true; courseTitleForLesson = directCheck.data.title || 'Course'; }
        ...
        // Path C: teacher-linked fallback — allow access to any published course
        if (!enrolledInCourse) {
          const courseCheck = await supabaseAdmin.from('courses').select('id,title').eq('id', lessonCourseId).eq('status', 'published').maybeSingle();
          if (!courseCheck.error && courseCheck.data) { enrolledInCourse = true; courseTitleForLesson = courseCheck.data.title || 'Course'; }
        }
      }
      if (!enrolledInCourse) {
...
<div style="font-size:11px;color:#94a3b8;">${footerNote}</div>
  ...
  // Student: submit (or resubmit) an assignment (poolQuery – bypasses PostgREST schema cache)
  app.post('/api/student/assignments/:assignmentId/submit', async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // poolQuery unavailable (ENOTFOUND / no DATABASE_URL) — fall back to supabaseAdmin
  ...
  app.post('/api/student/assignments/:assignmentId/save-draft', async (req: Request, res: Response) => {
    try {
      ...
      // Manual upsert via poolQuery — bypasses PostgREST schema cache entirely
  ...
  // Teacher: get all submissions for an assignment (poolQuery – bypasses PostgREST schema cache)
...
async function runAssignmentSubmissionsMigration() {
  ...
  // Notify PostgREST to reload schema cache so new tables are visible immediately
  try {
    await poolQuery(`SELECT pg_notify('pgrst', 'reload schema')`);
    console.log('[migration] PostgREST schema cache reloaded ✓');
  } catch { /* ignore if pg_notify not available */ }
  try {
